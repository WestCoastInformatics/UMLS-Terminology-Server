...
public class NeoplasmAnalysisMojo extends AbstractMojo {
	...
	final private String eclDesc = "<< 55342001";
...
//						Concept con = client.getConcept(Long.parseLong(columns[0]), null, authToken);
					...
					System.out.println("Failed processing: " + line + " with exception: " + e.getMessage());
				}
	...
	private void processDesc(String conId, String desc, PrintWriter outputDescFile) throws Exception {
		// TODO
		// 2) malignant tumor of * malignant tumour of---> NO
		// 7) Handle greater vestibular (Bartholin's) gland???
		// Lymph Node handling
		// Body Structure i.e. "of anterior wall of x" should not have "anterior wall"
		// (as body Struct) and "x" (as secondary),
		// --> it should have "anterior wall of x" as body structure
		// 'Pathology Fixes' --> To should be treated same as "of" in these cases. Need
			...
			if (splitKeyWord != null && desc.contains(splitKeyWord)) {
				...
				if (splitKeyWord.equals(" of ")) {
					...
					if (desc.startsWith("Local recurrence of")) {
						bodyStructIdx = desc.substring("Local recurrence of".length()).indexOf(" of ")
								+ "Local recurrence of".length();
					}

					// Ignore 'of' overlapping lesion
					if (desc.substring(bodyStructIdx).toLowerCase().contains("of overlapping lesion")) {
						bodyStructIdx = bodyStructIdx
								+ desc.substring(bodyStructIdx + "of overlapping lesion".length()).indexOf(" of ")
				...
				if (FALSE_POSITIVE_BODY_STRUCTURES
						.contains(desc.substring(desc.indexOf("of") + "of".length()).trim())) {
					containsBodyStructure = false;
				}
	 ...
	 */
	private PrintWriter prepareDescOutputFile() throws FileNotFoundException, UnsupportedEncodingException {
	 ...
	 *             the unsupported encoding exception
	 */
	private PrintWriter prepareRelOutputFile() throws FileNotFoundException, UnsupportedEncodingException {
