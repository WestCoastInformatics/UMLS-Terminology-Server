...
-- create index x_cdr_t on code_tree_relationships (terminology); -- WHERE IS TABLE CREATED?
...
CREATE VIEW ambig_concepts AS SELECT DISTINCT c1.id conceptId1, c2.id conceptId2 FROM concepts c1, concepts_atoms ca1, atoms a1, concepts c2, concepts_atoms ca2, atoms a2 WHERE c1.terminology = 'NCIMTH' AND c2.terminology = 'NCIMTH' AND c1.id = ca1.concepts_id AND ca1.atoms_id = a1.id AND c2.id = ca2.concepts_id AND ca2.atoms_id = a2.id AND c1.id < c2.id AND a1.lowerNameHash = a2.lowerNameHash AND a1.id != a2.id AND a1.publishable = 1 AND a2.publishable = 1;

-- Fact tables for helping speed up queries that rely on deep_relationships
--ORACLE
CREATE TABLE deep_atom_relationships2 (relationship_id NUMBER(20), component_type VARCHAR(255), terminologyId VARCHAR(255), terminology VARCHAR(255), version VARCHAR(255), relationshipType VARCHAR(255), additionalRelationshipType VARCHAR(255), obsolete NUMBER(1), suppressible NUMBER(1), published NUMBER(1), publishable NUMBER(1), workflowStatus VARCHAR(255), lastModifiedby VARCHAR(255), lastModified TIMESTAMP, from_atoms_id NUMBER(20), to_atoms_id NUMBER(20));
CREATE INDEX deep_atom_relationships2_from_atoms_id ON deep_atom_relationships2 (from_atoms_id);
CREATE INDEX deep_atom_relationships2_to_atoms_id ON deep_atom_relationships2 (to_atoms_id);

CREATE TABLE deep_concept_relationships2 (relationship_id NUMBER(20), component_type VARCHAR(255), terminologyId VARCHAR(255), terminology VARCHAR(255), version VARCHAR(255), relationshipType VARCHAR(255), additionalRelationshipType VARCHAR(255), obsolete NUMBER(1), suppressible NUMBER(1), published NUMBER(1), publishable NUMBER(1), workflowStatus VARCHAR(255), lastModifiedby VARCHAR(255), lastModified TIMESTAMP, from_concepts_id NUMBER(20), to_concepts_id NUMBER(20));
CREATE INDEX deep_concept_relationships2_from_concepts_id ON deep_concept_relationships2 (from_concepts_id);
CREATE INDEX deep_concept_relationships2_to_concepts_id ON deep_concept_relationships2 (to_concepts_id);