...
tsApp.controller('EditCtrl', [
  ...
  function($scope, $http, $location, gpService, utilService, tabService, securityService,
    workflowService, utilService, configureService, projectService, reportService, 
   ...
   // Workflow Bins Changed handler
    ...
    $scope.getWorklists = function() {
      if ($scope.mode == 'Assigned') {
        $scope.getAssignedWorklists($scope.currentProject.id, $scope.projectRole);
      } else if ($scope.mode == 'Available') {
        $scope.getAvailableWorklists($scope.currentProject.id, $scope.projectRole);
      } else if ($scope.mode == 'Checklists') {
        $scope.findChecklists($scope.currentProject.id, $scope.projectRole);
      }
    }
    ...
    $scope.getAvailableWorklists = function(projectId) {
      ...
      workflowService.findAvailableWorklists(projectId, $scope.user.userName, $scope.projectRole, pfs).then(function(response) {
    ...
    $scope.getAssignedWorklists = function(projectId, role) {
      ...
      workflowService.findAssignedWorklists(projectId, $scope.user.userName, role, pfs).then(function(response) {
        $scope.worklists = response.worklists;
        $scope.worklists.totalCount = $scope.worklists.length;
        resetSelected();
      });
    ...
    $scope.selectRecord = function(record) {
      ...
      for (var i=0; i<record.concepts.length; i++) {
        contentService.getConcept(record.concepts[i].id, $scope.currentProject.id).then(
          function(data) {
    ...
    $scope.performWorkflowAction = function(worklist){
      workflowService.performWorkflowAction($scope.currentProject.id, worklist.id, $scope.user.userName,
        $scope.currentProject.userRoleMap[$scope.user.userName], 'FINISH').then(
      // Success
      function(data) {
        $scope.initialize();
      },
    ...
    $scope.assignWorklistToSelf = function(worklist) {
        workflowService.performWorkflowAction($scope.currentProject.id, worklist.id, $scope.user.userName,
          $scope.currentProject.userRoleMap[$scope.user.userName], 'ASSIGN').then(
        // Success
        function(data) {
    ...
    $scope.unassignWorklist = function(worklist) {
      ...
      workflowService.performWorkflowAction($scope.currentProject.id, worklist.id,
        editor, $scope.currentProject.userRoleMap[$scope.user.userName], 'UNASSIGN').then(
      // Success
      function(data) {
        $scope.getWorklists();
      });
    ...
    $scope.approveConcept = function(concept) {
      contentService.getConcept(concept.id, $scope.currentProject.id).then(
        function(data) {
          var concept = data;
          metaEditingService.approveConcept($scope.currentProject.id, concept, false).then(
            // Success
            function(data) {
    ...
    $scope.openAddConceptModal = function(lrecord) {
      ...
      // Success
      function(data) {
        contentService.getConcept(data.id, $scope.currentProject.id).then(
          function(data) {
            $scope.data.push(data);
          },
          function(data) {
