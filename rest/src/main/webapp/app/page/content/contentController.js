...
tsApp
  .controller(
    ...
    [
      ...
      'configureService',
      'appConfig',
      function($scope, $routeParams, $http, $uibModal, $location, $q, $anchorScroll, $sce,
        gpService, utilService, tabService, securityService, metadataService, contentService,
        configureService, appConfig) {
        ...
        $scope.findComponentsAsList = function(loadFirst) {
          ...
          var hasQuery = $scope.searchParams && $scope.searchParams.query && $scope.searchParams.query.length > 0;
          var hasExpr = $scope.searchParams && $scope.searchParams.advancedMode && $scope.searchParams.expression && $scope.searchParams.expression.length > 0;

          // ensure query/expression string has appropriate length
          if (!hasQuery && !hasExpr) {
            alert("You must use at least one character to search" + ($scope.searchParams.advancedMode ? " or supply an expression" : ""));
            return;
          }
