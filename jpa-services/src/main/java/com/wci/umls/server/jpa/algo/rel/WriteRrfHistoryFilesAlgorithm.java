...
public class WriteRrfHistoryFilesAlgorithm extends AbstractAlgorithm {
  ...
  public void compute() throws Exception {
    ...
    javax.persistence.Query query =
        manager.createQuery("select count(*) from ConceptJpa c "
            + "where c.publishable = false and terminology = :terminology");
  ...
  private void writeMrcui(Concept c) throws Exception {
    // 0 CUI1    Unique identifier for first concept - Retired CUI - was present in some prior release, but is currently missing
    // 1 VER The last release version in which CUI1 was a valid CUI
    // 2 REL Relationship
    // 3 RELA    Relationship attribute
    // 4 MAPREASON   Reason for mapping
    // 5 CUI2    Unique identifier for second concept - the current CUI that CUI1 most closely maps to
    // 6 MAPIN   Is this map in current subset? Values of Y, N, or null. MetamorphoSys generates the Y or N to indicate whether the CUI2 concept is or is not present in the subset. The null value is for rows where the CUI1 was not present to begin with (i.e., REL=DEL).
    // e.g.
    // C0000401|1993AA|DEL|||||
    ...
    queryStr = "select distinct lastReleaseCui, c.terminologyId cui " + 
    ...
    for (Object[] objArray : results) {
      ...
      Set<String> cuis = (Set<String>)objArray[1]; // TODO look at this cast
      atomsMoved.put(lastReleaseCui, cuis);
    }
    
    // Determine "merge" cases - all keys from atomsMoved where the value is size()==1 and the key is not in currentCuis.
    ...
    for (Entry<String, Set<String>> entry : atomsMoved.entrySet()) {
      ...
      if (entry.getValue().size() == 1 && !currentCuis.contains(lastReleaseCui)) {
        ...
        //write an SY row where CUI1 is the last release CUI, and CUI2 is the single cui in the value.
        //save these CUI1->CUI2 pairs for later. 
        syCui1Cui2Pairs.put(lastReleaseCui, cui2);
        StringBuilder sb = new StringBuilder();
        sb.append(lastReleaseCui).append("|"); // 0 CUI1 
        sb.append(getProject().getVersion()).append("|"); // 1 VER
        sb.append("SY").append("|"); // 2 REL
        // 3 RELA
        // 4 MAPREASON
        sb.append(entry.getValue()).append("|"); // 5 CUI2
        // 6 MAPIN
        sb.append("\n");
        writerMap.get("MRCUI.RRF").print(sb.toString());
      }
    }
    //Determine "split" cases - all keys from atomsMoved where the value is size()>1 and the key is not in current cuis.
    ...
    if (c.getComponentHistory() == null || c.getComponentHistory().isEmpty()) {
      // If no bequeathal rel, -> write out a "DEL" entry to MRCUI
      List<String> bequeathalRels = getBequeathalRels(c);
      ...
      // If bequeathal rel -> write out bequeathal entry for each rel
        ...
        for(String bequeathalRel : bequeathalRels) {
          StringBuilder sb = new StringBuilder();
          sb.append(c.getTerminologyId()).append("|"); // 0 CUI1 
          sb.append(getProject().getVersion()).append("|"); // 1 VER
          sb.append(bequeathalRel).append("|"); // 2 REL
          // 3 RELA
          // 4 MAPREASON
          // 5 CUI2
          // 6 MAPIN
    ...
    // If it has a component history. (e.g. component.getComponentHistory().size()>0) - i.e. this is a "historical" concept.
      ...
      for (ComponentHistory history : c.getComponentHistory()) {
        ...
        if (history.getRelationshipType().equals("DEL")) {
          ...
          sb.append("DEL").append("|"); // 2 REL
          sb.append(history.getAdditionalRelationshipType()).append("|");// 3 RELA
          sb.append(history.getReason()).append("|"); // 4 MAPREASON
          sb.append(history.getReferencedConcept().getTerminologyId()).append("|"); // 5 CUI2 
          // 6 MAPIN
          sb.append("\n");
          writerMap.get("MRCUI.RRF").print(sb.toString());
        }  
        // If SY or R?     
          ...
          // if "referenced concept" is publishable, write it out as is // TODO? correct
          ...
          // If not publishable, check whether the referencedConcept has a terminologyId matching the CUI1 of SY rows computed above. 
          else {
            // If so, change the referencedConcept to point to the CUI2 concept.
            if (syCui1Cui2Pairs.containsKey(history.getReferencedConcept().getTerminologyId())) {
              // TODO
            }
          }
          // otherwise, don't write a MRCUI entry. (e.g this is a component history that will go away.)
        }
      } // end for
    }
    // sort file.
  ...
  private List<String> getBequeathalRels(Concept c) {
    List<String> bequeathalRels = new ArrayList<>();
    for (ConceptRelationship rel : c.getRelationships()) {
      if (rel.getRelationshipType().equals("BRO") || rel.getRelationshipType().equals("BRN") ||
        ...
        bequeathalRels.add(rel.getRelationshipType().substring(1));
      }
    }
  ...
  private void writeMraui() throws Exception {
    ...
    javax.persistence.Query query = null;
    queryStr =
        "select value(aid) aui, value(cid) lastReleaseCui, c.terminologyId cui "
            + "from ConceptJpa c join c.atoms a join a.conceptTerminologyIds cid "
    ...
    for (Object[] objArray : results) {
      ...
      sb.append(getProject().getVersion()).append("|"); // 2 VER
      // 3 REL
      // 4 RELA
      // 5 MAPREASON
      // 6 AUI2
      sb.append(cui).append("|"); // 7 CUI2
      // 8 MAPIN
      sb.append("\n");
