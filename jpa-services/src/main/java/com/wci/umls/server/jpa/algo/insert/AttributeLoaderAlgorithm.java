...
public class AttributeLoaderAlgorithm extends AbstractInsertMaintReleaseAlgorithm {
	...
	public void compute() throws Exception {
		...
		final IdentifierAssignmentHandler handler = newIdentifierAssignmentHandler(getProject().getTerminology());
		handler.setTransactionPerOperation(false);
		handler.beginTransaction();

		// Set up the search handler
		final ComputePreferredNameHandler prefNameHandler = getComputePreferredNameHandler(
		...
		try {
			...
			//
			final List<String> lines = loadFileIntoStringList(getSrcDirFile(), "attributes.src", null,
			...
			// reloading.
			if (replace) {
				final Set<Pair<String, String>> terminologyAttributesToRemove = new HashSet<>();

				for (final String line : lines) {

					FieldedStringTokenizer.split(line, "|", 14, fields);
					final Pair<String, String> terminologyAttribute = new ImmutablePair<>(fields[5], fields[3]);
				...
				for (Pair<String, String> terminologyAttribute : terminologyAttributesToRemove) {
					...
					logInfo("  Removing existing attributes where terminology= " + terminologyAndVersion + ", name ="
							+ attributeName);
					...
					final List<Class> containingClazzes = new ArrayList<>(
							Arrays.asList(AtomJpa.class, ConceptJpa.class));

					for (Class clazz : containingClazzes) {
						final String query = "SELECT a.id, att.id from " + clazz.getSimpleName()
								+ " a join a.attributes att where att.terminology=:specifiedTerminology and att.version=:specifiedVersion and att.name=:name";
						final Map<String, String> params = getDefaultQueryParams(getProject());
						...
						jpaQuery.setParameter("specifiedTerminology", terminology.getTerminology());
						...
						for (Object[] entry : list) {
							...
							final ComponentHasAttributes containingComponent = (ComponentHasAttributes) getComponent(
									containingComponentId, clazz);
			...
			for (final String line : lines) {
				...
				if (setTerminology == null) {
					logWarnAndUpdate(line, "WARNING - terminology not found: " + fields[5] + ".");
					continue;
				}
				...
				if (fields[3].equals("DEFINITION")) {
					...
					final ComponentHasDefinitions containerComponent = (ComponentHasDefinitions) getComponent(
							fields[10], fields[1],
							fields[11].equals("") ? null : getCachedTerminology(fields[11]).getTerminology(), null);
					...
					if (containerComponent instanceof Atom) {
						...
						final List<Atom> atoms = prefNameHandler.sortAtoms(atomClass.getAtoms(),
								getPrecedenceList(getProject().getTerminology(), getProject().getVersion()));
						atom = atoms.get(0);
					} else {
						logWarnAndUpdate(line,
								"WARNING - " + containerComponent.getClass().getName() + " is an unhandled type.");
						continue;
					}

					// Compute definition identity
					final String newDefinitionAtui = handler.getTerminologyId(newDefinition, atom);

					// Check to see if attribute with matching ATUI already
					// exists in the
					// database
					final Definition oldDefinition = (Definition) getComponent("DEFINITION", newDefinitionAtui,
					...
					if (oldDefinition == null) {
						newDefinition.getAlternateTerminologyIds().put(getProject().getTerminology(),
								newDefinitionAtui);
						final Definition newDefinition2 = addDefinition(newDefinition, containerComponent);
					...
					// object.
						...
						oldDefinition.getAlternateTerminologyIds().put(getProject().getTerminology(),
								newDefinitionAtui);

						// Update the version
						if (!oldDefinition.getVersion().equals(newDefinition.getVersion())) {
						...
						if (!oldDefinition.equals(newDefinition)) {
							...
							if (oldDefinition.isSuppressible() != newDefinition.isSuppressible()) {
				...
				// Otherwise, process the line as an attribute
					...
					// Load the containing object
					final ComponentHasAttributes containerComponent = (ComponentHasAttributes) getComponent(fields[10],
							fields[1], getCachedTerminologyName(fields[11]), null);
					if (containerComponent == null) {
						logWarnAndUpdate(line, "WARNING - could not find Component for type: " + fields[10]
								+ ", terminologyId: " + fields[1] + ", and terminology:" + fields[11]);
						continue;
					...
					final String newAttributeAtui = handler.getTerminologyId(newAttribute, containerComponent);

					// Check to see if attribute with matching ATUI already
					// exists in the
					// database
					final Attribute oldAttribute = (Attribute) getComponent("ATUI", newAttributeAtui,
							newAttribute.getTerminology(), null);

					// If no attribute with the same ATUI exists, create this
					// new
					// Attribute, and add it to its containing component
					if (oldAttribute == null) {
						newAttribute.getAlternateTerminologyIds().put(getProject().getTerminology(), newAttributeAtui);
						final Attribute newAttribute2 = addAttribute(newAttribute, containerComponent);
					...
					// object.
						...
						oldAttribute.getAlternateTerminologyIds().put(getProject().getTerminology(), newAttributeAtui);
						...
						if (!oldAttribute.equals(newAttribute)) {
							...
							if (oldAttribute.isSuppressible() != newAttribute.isSuppressible()) {
