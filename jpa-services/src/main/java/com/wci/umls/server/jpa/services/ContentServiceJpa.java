...
public class ContentServiceJpa extends MetadataServiceJpa implements ContentService {
	...
	private static void init() {
		try {
			if (ConfigUtility.getConfigProperties().containsKey("javax.persistence.query.timeout")) {
				queryTimeout = Integer
						.parseInt(ConfigUtility.getConfigProperties().getProperty("javax.persistence.query.timeout"));
			}
			...
			for (final String handlerName : config.getProperty(key).split(",")) {
				if (handlerName.isEmpty())
					continue;
				// Add handlers to map
				final IdentifierAssignmentHandler handlerService = ConfigUtility
						.newStandardHandlerInstanceWithConfiguration(key, handlerName,
			...
			if (!idHandlerMap.containsKey(ConfigUtility.DEFAULT)) {
				throw new Exception(
						"identifier.assignment.handler." + ConfigUtility.DEFAULT + " expected and does not exist.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			idHandlerMap = null;
		...
		try {
			...
			for (final String handlerName : config.getProperty(key).split(",")) {
				...
				// Add handlers to map
				final ComputePreferredNameHandler handlerService = ConfigUtility
						.newStandardHandlerInstanceWithConfiguration(key, handlerName,
			...
			if (!pnHandlerMap.containsKey(ConfigUtility.DEFAULT)) {
				throw new Exception(
						"compute.preferred.name.handler." + ConfigUtility.DEFAULT + " expected and does not exist.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			pnHandlerMap = null;
		...
		try {
			...
			final NormalizedStringHandler handlerService = ConfigUtility
					.newStandardHandlerInstanceWithConfiguration(key, handlerName, NormalizedStringHandler.class);
	...
	@Override
	public ConceptList getConcepts(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get concepts " + terminologyId + "/" + terminology + "/" + version);
		final List<Concept> concepts = getComponents(terminologyId, terminology, version, ConceptJpa.class);
		if (concepts == null) {
			return null;
		}
	...
	public Concept getConcept(String terminologyId, String terminology, String version, String branch)
		...
		Logger.getLogger(getClass()).debug(
				"Content Service - get concept " + terminologyId + "/" + terminology + "/" + version + "/" + branch);

		final Concept concept = getComponent(terminologyId, terminology, version, branch, ConceptJpa.class);
	...
	public void updateConcept(Concept concept) throws Exception {
		...
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(concept.getTerminology());
		if (assignIdentifiersFlag) {
	...
	public Subset getSubset(String terminologyId, String terminology, String version, String branch,
		...
		Logger.getLogger(getClass()).debug(
				"Content Service - get subset " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
		if (subsetClass != null) {
			return getComponent(terminologyId, terminology, version, branch, subsetClass);
		} else {
			Subset subset = getComponent(terminologyId, terminology, version, branch, AtomSubsetJpa.class);
			if (subset == null) {
	...
	/* see superclass */
	@Override
	public SubsetList getAtomSubsets(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get atom subsets " + terminology + "/" + version);
		final javax.persistence.Query query = manager.createQuery(
				"select a from AtomSubsetJpa a where " + "version = :version and terminology = :terminology");
	...
	/* see superclass */
	@Override
	public SubsetList getConceptSubsets(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get concept subsets " + terminology + "/" + version);
		final javax.persistence.Query query = manager.createQuery(
				"select a from ConceptSubsetJpa a where " + "version = :version and terminology = :terminology");
	...
	@Override
	public SubsetMemberList findAtomSubsetMembers(String subsetId, String terminology, String version, String branch,
			String query, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find atom subset members " + subsetId + "/" + terminology
				+ "/" + version + ", query=" + query);
		...
		list.setObjects((List) searchHandler.getQueryResults(terminology, version, branch, finalQuery.toString(),
				"memberNameSort", AtomSubsetMemberJpa.class, pfs, totalCt, manager));
	...
	@Override
	public SubsetMemberList findConceptSubsetMembers(String subsetId, String terminology, String version, String branch,
			String query, PfsParameter pfs) throws Exception {
	...
	public SubsetMemberList getAtomSubsetMembers(String atomId, String terminology, String version, String branch) {
		Logger.getLogger(getClass())
				.debug("Content Service - get subset members for atom " + atomId + "/" + terminology + "/" + version);
		final javax.persistence.Query query = manager
				.createQuery("select a from AtomSubsetMemberJpa a, " + " AtomJpa b where b.terminologyId = :atomId "
		...
		try {
			...
			/*
			 * for (final SubsetMember<? extends ComponentHasAttributesAndName> s : list
			 * .getObjects()) { if (s.getAttributes() != null) s.getAttributes().size(); }
			 */
	...
	@SuppressWarnings("unchecked")
	@Override
	public SubsetMemberList getConceptSubsetMembers(String conceptId, String terminology, String version,
			String branch) {
		Logger.getLogger(getClass()).debug(
				"Content Service - get subset members for concept " + conceptId + "/" + terminology + "/" + version);
		final javax.persistence.Query query = manager.createQuery(
				"select a from ConceptSubsetMemberJpa a, " + " ConceptJpa b where b.terminologyId = :conceptId "
						+ "and b.version = :version " + "and b.terminology = :terminology and a.member = b");
	...
	/* see superclass */
	@Override
	public SubsetList getAllSubsets(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass())
	...
	@SuppressWarnings("unchecked")
	@Override
	public DefinitionList getDefinitions(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get definitions " + terminologyId + "/" + terminology + "/" + version);
		final List<Definition> definitions = getComponents(terminologyId, terminology, version, DefinitionJpa.class);
		if (definitions == null) {
			return null;
		}
	...
	/* see superclass */
	@Override
	public Definition getDefinition(String terminologyId, String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get definition " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
		return getComponent(terminologyId, terminology, version, branch, DefinitionJpa.class);
	}

	/* see superclass */
	@Override
	public Definition addDefinition(Definition definition, ComponentHasDefinitions component) throws Exception {
	...
	/* see superclass */
	@Override
	public void updateDefinition(Definition definition, ComponentHasDefinitions component) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - update definition " + definition);

		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(definition.getTerminology());
		if (assignIdentifiersFlag) {
	...
	/* see superclass */
	@Override
	public SemanticTypeComponent addSemanticTypeComponent(SemanticTypeComponent component, Concept concept)
			throws Exception {
	...
	/* see superclass */
	@Override
	public void updateSemanticTypeComponent(SemanticTypeComponent semanticTypeComponent, Concept concept)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - update semanticTypeComponent " + semanticTypeComponent);

		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(
				semanticTypeComponent.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
				SemanticTypeComponent semanticTypeComponent2 = getComponent(semanticTypeComponent.getId(),
				...
				semanticTypeComponent.setTerminologyId(idHandler.getTerminologyId(semanticTypeComponent, concept));
			}
		}
		// update component
	...
	@SuppressWarnings("unchecked")
	@Override
	public DescriptorList getDescriptors(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get descriptors " + terminologyId + "/" + terminology + "/" + version);
		final List<Descriptor> descriptors = getComponents(terminologyId, terminology, version, DescriptorJpa.class);
		if (descriptors == null) {
			return null;
		}
	...
	/* see superclass */
	@Override
	public Descriptor getDescriptor(String terminologyId, String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get descriptor " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
	...
	public void updateDescriptor(Descriptor descriptor) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(descriptor.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
				Descriptor descriptor2 = getDescriptor(descriptor.getId());
				if (!idHandler.getTerminologyId(descriptor).equals(idHandler.getTerminologyId(descriptor2))) {
					throw new Exception("Update cannot be used to change object identity.");
				}
			} else {
	...
	@SuppressWarnings("unchecked")
	@Override
	public CodeList getCodes(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
	...
	/* see superclass */
	@Override
	public Code getCode(String terminologyId, String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get code " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
	...
	public void updateCode(Code code) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(code.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public LexicalClassList getLexicalClasses(String terminologyId, String terminology, String version)
			throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get lexical classes " + terminologyId + "/" + terminology + "/" + version);
		final List<LexicalClass> luis = getComponents(terminologyId, terminology, version, LexicalClassJpa.class);
		if (luis == null) {
			return null;
		}
	...
	/* see superclass */
	@Override
	public LexicalClass getLexicalClass(String terminologyId, String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get lexical class " + terminologyId + "/" + terminology
				+ "/" + version + "/" + branch);
	...
	public void updateLexicalClass(LexicalClass lexicalClass) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(lexicalClass.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
				LexicalClass lexicalClass2 = getLexicalClass(lexicalClass.getId());
				if (!idHandler.getTerminologyId(lexicalClass).equals(idHandler.getTerminologyId(lexicalClass2))) {
					throw new Exception("Update cannot be used to change object identity.");
				}
			} else {
	...
	@SuppressWarnings("unchecked")
	@Override
	public StringClassList getStringClasses(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get string classes " + terminologyId + "/" + terminology + "/" + version);
		final List<StringClass> suis = getComponents(terminologyId, terminology, version, StringClassJpa.class);
		if (suis == null) {
			return null;
		}
	...
	/* see superclass */
	@Override
	public StringClass getStringClass(String terminologyId, String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get string class " + terminologyId + "/" + terminology
				+ "/" + version + "/" + branch);
	...
	public void updateStringClass(StringClass stringClass) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(stringClass.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
				StringClass stringClass2 = getStringClass(stringClass.getId());
				if (!idHandler.getTerminologyId(stringClass).equals(idHandler.getTerminologyId(stringClass2))) {
					throw new Exception("Update cannot be used to change object identity.");
				}
			} else {
	...
	/* see superclass */
	@Override
	public ConceptList findDescendantConcepts(String terminologyId, String terminology, String version,
		...
		@SuppressWarnings("unchecked")
		final List<Concept> descendants = findDescendantsHelper(terminologyId, terminology, version, childrenOnly,
	...
	@Override
	public AtomList findDescendantAtoms(String terminologyId, String terminology, String version, boolean childrenOnly,
			String branch, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find descendant atoms " + terminologyId + ", " + terminology);
		final long[] totalCt = new long[1];
		@SuppressWarnings("unchecked")
		final List<Atom> descendants = findDescendantsHelper(terminologyId, terminology, version, childrenOnly, branch,
				pfs, AtomJpa.class, totalCt);
	...
	@Override
	public AtomList findAncestorAtoms(String terminologyId, String terminology, String version, boolean parentsOnly,
			String branch, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find ancestor atoms " + terminologyId + ", " + terminology);
		final long[] totalCt = new long[1];
		@SuppressWarnings("unchecked")
		final List<Atom> ancestors = findAncestorsHelper(terminologyId, terminology, version, parentsOnly, branch, pfs,
				AtomJpa.class, totalCt);
		final AtomList list = new AtomListJpa();
		list.setObjects(ancestors);
		list.setTotalCount((int) totalCt[0]);
		return list;
	...
	private List findDescendantsHelper(String terminologyId, String terminology, String version, boolean childrenOnly,
			String branch, PfsParameter pfs, Class<?> clazz, long[] totalCt) throws Exception {
		...
		final String queryStr = "select a from " + clazz.getName().replace("Jpa", "TransitiveRelationshipJpa") + " tr, "
				+ clazz.getName() + " super, " + clazz.getName() + " a " + " where super.version = :version "
		...
		final javax.persistence.Query ctQuery = manager.createQuery(
				"select count(*) from " + clazz.getName().replace("Jpa", "TransitiveRelationshipJpa") + " tr, "
						+ clazz.getName() + " super, " + clazz.getName() + " a " + " where super.version = :version "
						+ " and super.terminology = :terminology " + " and super.terminologyId = :terminologyId"
						+ " and tr.superType = super" + " and tr.subType = a " + " and tr.superType != tr.subType"
						+ (childrenOnly ? " and depth = 1" : ""));
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private List findAncestorsHelper(String terminologyId, String terminology, String version, boolean parentsOnly,
			String branch, PfsParameter pfs, Class<?> clazz, long[] totalCt) throws Exception {
		...
		final String queryStr = "select a from " + clazz.getName().replace("Jpa", "TransitiveRelationshipJpa") + " tr, "
				+ clazz.getName() + " sub, " + clazz.getName() + " a " + " where sub.version = :version "
		...
		final javax.persistence.Query ctQuery = manager.createQuery("select count(*) from "
				+ clazz.getName().replace("Jpa", "TransitiveRelationshipJpa") + " tr, " + clazz.getName() + " sub, "
				+ clazz.getName() + " a " + " where sub.version = :version " + " and sub.terminology = :terminology "
				+ " and sub.terminologyId = :terminologyId" + " and tr.subType = sub" + " and tr.superType = a "
				+ " and tr.subType != tr.superType" + (parentsOnly ? " and depth = 1" : ""));
	...
	/* see superclass */
	@Override
	public DescriptorList findDescendantDescriptors(String terminologyId, String terminology, String version,
			boolean childrenOnly, String branch, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find descendant descriptors " + terminologyId + ", " + terminology);
		long[] totalCt = new long[1];
		@SuppressWarnings("unchecked")
		final List<Descriptor> descendants = findDescendantsHelper(terminologyId, terminology, version, childrenOnly,
	...
	/* see superclass */
	@Override
	public DescriptorList findAncestorDescriptors(String terminologyId, String terminology, String version,
			boolean childrenOnly, String branch, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find ancestor descriptors " + terminologyId + ", " + terminology);
		long[] totalCt = new long[1];
		@SuppressWarnings("unchecked")
		final List<Descriptor> ancestors = findAncestorsHelper(terminologyId, terminology, version, childrenOnly,
				branch, pfs, DescriptorJpa.class, totalCt);
		final DescriptorList list = new DescriptorListJpa();
		list.setObjects(ancestors);
		list.setTotalCount((int) totalCt[0]);
	...
	/* see superclass */
	@Override
	public CodeList findDescendantCodes(String terminologyId, String terminology, String version, boolean childrenOnly,
			String branch, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find descendant codes " + terminologyId + ", " + terminology);
		long[] totalCt = new long[1];
		@SuppressWarnings("unchecked")
		final List<Code> descendants = findDescendantsHelper(terminologyId, terminology, version, childrenOnly, branch,
				pfs, CodeJpa.class, totalCt);
		final CodeList list = new CodeListJpa();
		list.setObjects(descendants);
		list.setTotalCount((int) totalCt[0]);
		return list;
	...
	/* see superclass */
	@Override
	public CodeList findAncestorCodes(String terminologyId, String terminology, String version, boolean parentsOnly,
			String branch, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find ancestor codes " + terminologyId + ", " + terminology);
		long[] totalCt = new long[1];
		@SuppressWarnings("unchecked")
		final List<Code> descendants = findAncestorsHelper(terminologyId, terminology, version, parentsOnly, branch,
				pfs, CodeJpa.class, totalCt);
		final CodeList list = new CodeListJpa();
		list.setObjects(descendants);
		list.setTotalCount((int) totalCt[0]);
	...
	@SuppressWarnings("unchecked")
	@Override
	public AtomList getAtoms(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
	...
	/* see superclass */
	@Override
	public Atom getAtom(String terminologyId, String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get atom " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
	...
	public void updateAtom(Atom atom) throws Exception {
		...
		// Id assignment
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(atom.getTerminology());
	...
	/* see superclass */
	@Override
	public void moveAtoms(Concept fromConcept, Concept toConcept, List<Atom> fromAtoms) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - move atoms " + fromAtoms + " from concept " + fromConcept
				+ " to concept " + toConcept);
	...
	/* see superclass */
	@Override
	public ConceptRelationship createInverseConceptRelationship(ConceptRelationship relationship) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - create inverse of concept relationship " + relationship);
		if (relationship != null) {
			...
			inverseRelationship.setRelationshipType(getRelationshipType(relationship.getRelationshipType(),
					relationship.getTerminology(), relationship.getVersion()).getInverse().getAbbreviation());
	...
	public RelationshipList getInverseRelationships(String terminology, String version,
		...
		Logger.getLogger(getClass()).debug("Content Service - get the inverse of concept relationship " + relationship);

		// Relationship<? extends ComponentInfo, ? extends ComponentInfo>
		// inverseRelationship = null;

		if (relationship != null) {
			final String inverseRelType = getRelationshipType(relationship.getRelationshipType(), terminology, version)
					.getInverse().getAbbreviation();

			final String inverseAdditionalRelType = getAdditionalRelationshipType(
					relationship.getAdditionalRelationshipType(), terminology, version).getInverse().getAbbreviation();

			final RelationshipList relList = findRelationshipsForComponentHelper(null, null, null, Branch.ROOT,
					"fromId:" + relationship.getTo().getId() + " AND toId:" + relationship.getFrom().getId()
	...
	/* see superclass */
	@Override
	public String getInverseRelationshipType(String terminology, String version, String relationshipType)
			throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get the inverse for relationship type " + relationshipType);

		final String inverseRelType = getRelationshipType(relationshipType, terminology, version).getInverse()
				.getAbbreviation();
	...
	@Override
	public Relationship<? extends ComponentInfo, ? extends ComponentInfo> getInverseRelationship(String terminology,
			String version, Relationship<? extends ComponentInfo, ? extends ComponentInfo> relationship)
			throws Exception {
	...
	public Relationship<? extends ComponentInfo, ? extends ComponentInfo> getRelationship(Long id,
		...
		if (relationshipClass != null) {
			...
			Relationship<? extends ComponentInfo, ? extends ComponentInfo> rel = getComponent(id,
	...
	public RelationshipList getRelationships(String terminologyId, String terminology, String version,
			...
			throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - find relationships " + terminologyId + "/" + terminology + "/" + version);
		...
		if (relationshipClass != null) {
			...
			relationships = getComponents(terminologyId, terminology, version, ConceptRelationshipJpa.class);
			if (relationships == null) {
				relationships = getComponents(terminologyId, terminology, version, AtomRelationshipJpa.class);
			}
			if (relationships == null) {
				relationships = getComponents(terminologyId, terminology, version, CodeRelationshipJpa.class);
			}
			if (relationships == null) {
				relationships = getComponents(terminologyId, terminology, version, DescriptorRelationshipJpa.class);
			}
		}
	...
	/* see superclass */
	@Override
	public Relationship<? extends ComponentInfo, ? extends ComponentInfo> getRelationship(String terminologyId,
			String terminology, String version, String branch,
			Class<? extends Relationship<? extends ComponentInfo, ? extends ComponentInfo>> relationshipClass)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find relationship " + terminologyId + "/" + terminology
				+ "/" + version + "/" + branch);
		if (relationshipClass != null) {
			return getComponent(terminologyId, terminology, version, branch, relationshipClass);
		}
		final Relationship<? extends ComponentInfo, ? extends ComponentInfo> rel = getComponent(terminologyId,
				terminology, version, branch, ConceptRelationshipJpa.class);
		if (rel == null) {
	...
	public TransitiveRelationship<? extends AtomClass> getTransitiveRelationship(Long id,
			Class<? extends TransitiveRelationship<? extends AtomClass>> relationshipClass) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get transitive relationship " + id);
		if (relationshipClass != null) {
			return getComponent(id, relationshipClass);
		} else {
			TransitiveRelationship<? extends AtomClass> rel = getComponent(id, ConceptTransitiveRelationshipJpa.class);
			if (rel == null) {
	...
	/* see superclass */
	@Override
	public void updateTransitiveRelationship(TransitiveRelationship<? extends ComponentHasAttributes> rel)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - update transitive relationship " + rel);

		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(rel.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
				@SuppressWarnings("unchecked")
				final TransitiveRelationship<? extends ComponentHasAttributes> rel2 = getComponent(rel.getId(),
						rel.getClass());
	...
	public void removeTransitiveRelationship(Long id,
		...
		final TransitiveRelationship<? extends ComponentHasAttributes> rel = getComponent(id, relationshipClass);
		removeComponent(id, rel.getClass());
	}
	...
	@SuppressWarnings("rawtypes")
	@Override
	public TreePosition<?> getTreePosition(Long id, Class<? extends TreePosition> treeposClass) throws Exception {
	...
	public void updateTreePosition(TreePosition<?> treepos) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(treepos.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
	...
	@SuppressWarnings({ "rawtypes" })
	@Override
	public void removeTreePosition(Long id, Class<? extends TreePosition> treeposClass) throws Exception {
	...
	public void updateSubset(Subset subset) throws Exception {
		...
		// Id assignment
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(subset.getTerminology());
		if (!idHandler.allowIdChangeOnUpdate() && assignIdentifiersFlag) {
	...
	@Override
	public SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> getSubsetMember(Long id,
		...
		if (memberClass != null) {
			...
			SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> member = getComponent(id,
	...
	public SubsetMemberList getSubsetMembers(String terminologyId, String terminology, String version,
			...
			throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get subset members " + terminologyId + "/" + terminology + "/" + version);
	...
	/* see superclass */
	@Override
	public SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> getSubsetMember(String terminologyId,
			String terminology, String version, String branch,
			Class<? extends SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset>> memberClass)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get subset member " + terminologyId + "/" + terminology
				+ "/" + version + "/" + branch);
		if (memberClass != null) {
			return getComponent(terminologyId, terminology, version, branch, memberClass);
		} else {
			SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> member = getComponent(terminologyId,
					terminology, version, branch, AtomSubsetMemberJpa.class);
			if (member == null) {
				member = getComponent(terminologyId, terminology, version, branch, ConceptSubsetMemberJpa.class);
			}
			return member;
		}
	}
	...
	public SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> addSubsetMember(
			SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> subsetMember) throws Exception {
	...
	/* see superclass */
	@Override
	public void updateSubsetMember(SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> subsetMember)
			throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - update subsetMember " + subsetMember);
		// Id assignment
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(subsetMember.getTerminology());
		if (!idHandler.allowIdChangeOnUpdate() && assignIdentifiersFlag) {
			@SuppressWarnings("unchecked")
			SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> subsetMember2 = getComponent(
					subsetMember.getId(), subsetMember.getClass());
			if (!idHandler.getTerminologyId(subsetMember).equals(idHandler.getTerminologyId(subsetMember2))) {
				throw new Exception("Update cannot be used to change object identity.");
			}
		}
	...
	public void removeSubsetMember(Long id,
		...
		final SubsetMember<? extends ComponentHasAttributesAndName, ? extends Subset> member = getComponent(id,
				memberClass);
	...
	@SuppressWarnings("unchecked")
	@Override
	public AttributeList getAttributes(String terminologyId, String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get attributes " + terminologyId + "/" + terminology + "/" + version);
		final List<Attribute> attributes = getComponents(terminologyId, terminology, version, AttributeJpa.class);
		if (attributes == null) {
			return null;
		}
	...
	/* see superclass */
	@Override
	public Attribute getAttribute(String terminologyId, String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get attribute " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
	...
	public void updateAttribute(Attribute attribute, ComponentInfo component) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(attribute.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
	...
	/* see superclass */
	@Override
	public SearchResultList findConceptSearchResults(String terminology, String version, String branch, String query,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - find concept search results " + terminology + "/" + version + "/" + query);
		final SearchResultList results = findSearchResultsForQueryHelper(terminology, version, branch, query, pfs,
				ConceptJpa.class);
		return results;
	}
	...
	/* see superclass */
	@Override
	public ConceptList findConcepts(String terminology, String version, String branch, String query, PfsParameter pfs)
			throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - find concepts " + terminology + "/" + version + "/" + query);
		int[] totalCt = new int[1];
		final List<ConceptJpa> results = findForQueryHelper(terminology, version, branch, query, pfs, totalCt, null,
				ConceptJpa.class);
	...
	public StringList autocompleteConcepts(String terminology, String version, String searchTerm) throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - autocomplete concepts " + terminology + ", " + version + ", " + searchTerm);
		return autocompleteHelper(terminology, version, searchTerm, ConceptJpa.class);
	}

	/* see superclass */
	@Override
	public SearchResultList findDescriptorSearchResults(String terminology, String version, String branch, String query,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
	...
	/* see superclass */
	@Override
	public DescriptorList findDescriptors(String terminology, String version, String branch, String query,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - find concepts " + terminology + "/" + version + "/" + query);
		int[] totalCt = new int[1];
		final List<DescriptorJpa> results = findForQueryHelper(terminology, version, branch, query, pfs, totalCt, null,
				DescriptorJpa.class);
	...
	/* see superclass */
	@Override
	public StringList autocompleteDescriptors(String terminology, String version, String searchTerm) throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - autocomplete descriptors " + terminology + ", " + version + ", " + searchTerm);
	 ...
	 *             the exception
	 */
	public <T extends AtomClass> SearchResultList findSearchResultsForQueryHelper(String terminology, String version,
			String branch, String query, PfsParameter pfs, Class<T> clazz) throws Exception {
		// Prepare results
		int[] totalCt = new int[1];
		Map<Long, Float> scoreMap = new HashMap<>();
		final List<T> results = findForQueryHelper(terminology, version, branch, query, pfs, totalCt, scoreMap, clazz);
		...
		if (results != null) {
			for (final T r : results) {
				...
				if (ConceptJpa.class.isAssignableFrom(clazz)) {
					List<Atom> atoms = ((ConceptJpa) r).getAtoms();
					sr.setCodeId(atoms.iterator().next().getCodeId());
					sr.setLeafNode(atoms.iterator().next().isLeafNode());
					sr.setPostCoodinated(atoms.iterator().next().isHasPostCoordination());
				}
	 ...
	 *             the exception
	 */
	public <T extends AtomClass> List<T> findForQueryHelper(String terminology, String version, String branch,
			String query, PfsParameter pfs, int[] totalCt, Map<Long, Float> scoreMap, Class<T> clazz) throws Exception {
		...
		if (localPfs.getExpression() != null && !localPfs.getExpression().isEmpty()) {
			...
			if (exprResults.size() > 0) {
				String exprQueryRestr = (localPfs.getQueryRestriction() != null
						&& !localPfs.getQueryRestriction().isEmpty() ? " AND " : "") + "terminologyId:(";
				...
				exprQueryRestr = exprQueryRestr.substring(0, exprQueryRestr.length() - 1) + ")^" + exprResults.size();
		...
		if (exprResults == null || exprResults.size() > 0) {
			luceneResults = searchHandler.getQueryResults(terminology, version, branch, query, "atoms.nameSort", clazz,
					localPfs, totalCt, manager);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private <T extends AtomClass> SearchResultList findForGeneralQueryHelper(String luceneQuery, String JPQLQuery,
			String branch, PfsParameter pfs, Class<T> clazz) throws Exception {
		// Prepare results
		...
		if (luceneQuery != null && !luceneQuery.equals("")) {
			...
			luceneQueryClasses.addAll(searchHandler.getQueryResults(null, null, branch, luceneQuery, "atomsName.sort",
		...
		if (JPQLQueryFlag) {
			...
			if (pfs != null && pfs.getSortField() != null) {
				final Method getMethod = clazz.getMethod(
						"get" + pfs.getSortField().substring(0, 1).toUpperCase() + pfs.getSortField().substring(1));
	 ...
	 *             the exception
	 */
	private <T extends AtomClass> StringList autocompleteHelper(String terminology, String version, String searchTerm,
		...
		final QueryBuilder titleQB = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(clazz)
				.get();

		final Query query = titleQB.phrase().withSlop(2).onField(TITLE_NGRAM_INDEX).andField(TITLE_EDGE_NGRAM_INDEX)
				.boostedTo(5).andField("atoms.name").boostedTo(5).sentence(searchTerm.toLowerCase()).createQuery();
		...
		final FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(booleanQuery, clazz);
	...
	/* see superclass */
	@Override
	public SearchResultList findCodeSearchResults(String terminology, String version, String branch, String query,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).info("Content Service - find codes " + terminology + "/" + version + "/" + query);
		final SearchResultList results = findSearchResultsForQueryHelper(terminology, version, branch, query, pfs,
				CodeJpa.class);
		for (final SearchResult result : results.getObjects()) {
	...
	/* see superclass */
	@Override
	public CodeList findCodes(String terminology, String version, String branch, String query, PfsParameter pfs)
			throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - find concepts " + terminology + "/" + version + "/" + query);
		int[] totalCt = new int[1];
		final List<CodeJpa> results = findForQueryHelper(terminology, version, branch, query, pfs, totalCt, null,
				CodeJpa.class);
	...
	/* see superclass */
	@Override
	public StringList autocompleteCodes(String terminology, String version, String searchTerm) throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - autocomplete codes " + terminology + ", " + version + ", " + searchTerm);
	...
	public ConceptList getAllConcepts(String terminology, String version, String branch) {
		...
		try {
			final javax.persistence.Query query = manager.createQuery(
					"select a from ConceptJpa a " + "where version = :version " + "and terminology = :terminology "
	...
	/* see superclass */
	@Override
	public List<Long> getAllConceptIds(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get all concept ids " + terminology + "/" + version + "/" + branch);
		assert branch != null;
	...
	public List<Long> getAmbiguousAtomIds(Concept concept) {
		try {
			...
			final Set<String> lowerNameHashes = concept.getAtoms().stream().map(a -> a.getLowerNameHash())
			...
			final javax.persistence.Query query = manager
					.createQuery("select distinct a.lowerNameHash from ConceptJpa c join c.atoms a "
			...
			return concept.getAtoms().stream().filter(a -> ambigLowerNameHashes.contains(a.getLowerNameHash()))
	...
	public DescriptorList getAllDescriptors(String terminology, String version, String branch) {
		Logger.getLogger(getClass())
				.debug("Content Service - get all descriptors " + terminology + "/" + version + "/" + branch);
		assert branch != null;

		try {
			final javax.persistence.Query query = manager.createQuery(
					"select a from DescriptorJpa a " + "where version = :version " + "and terminology = :terminology "
							+ "and (branch = :branch or branchedTo not like :branchMatch)");
			query.setParameter("terminology", terminology);
			query.setParameter("version", version);
			query.setParameter("branch", branch);
	...
	@Override
	public List<Long> getAllDescriptorIds(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get all descriptor ids " + terminology + "/" + version + "/" + branch);
		assert branch != null;

		final Map<String, String> params = new HashMap<>();
		params.put("terminology", terminology);
		params.put("version", version);
		return executeSingleComponentIdQuery(null, QueryType.LUCENE, params, DescriptorJpa.class, false);
	}
	...
	public CodeList getAllCodes(String terminology, String version, String branch) {
		...
		try {
			final javax.persistence.Query query = manager.createQuery(
					"select a from CodeJpa a " + "where version = :version " + "and terminology = :terminology "
							+ "and (branch = :branch or branchedTo not like :branchMatch)");
			query.setParameter("terminology", terminology);
			query.setParameter("version", version);
			query.setParameter("branch", branch);
	...
	/* see superclass */
	@Override
	public List<Long> getAllCodeIds(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass())
	...
	public void clearTransitiveClosure(String terminology, String version) throws Exception {
		Logger.getLogger(getClass())
				.info("Content Service - Clear transitive closure data for " + terminology + ", " + version);
		try {
			...
			javax.persistence.Query query = manager.createQuery("DELETE From ConceptTransitiveRelationshipJpa "
			...
			query.setParameter("version", version);
			deleteRecords = query.executeUpdate();
			Logger.getLogger(getClass()).info("    CodeTransitiveRelationshipJpa records deleted = " + deleteRecords);
	...
	public void clearTreePositions(String terminology, String version) throws Exception {
		...
		try {
			...
			query = manager.createQuery("DELETE From CodeTreePositionJpa " + " c where terminology = :terminology "
					+ " and version = :version");
			query.setParameter("terminology", terminology);
			query.setParameter("version", version);
			deleteRecords = query.executeUpdate();
	...
	/* see superclass */
	@Override
	public IdentifierAssignmentHandler getIdentifierAssignmentHandler(String terminology) throws Exception {
	...
	/* see superclass */
	@Override
	public IdentifierAssignmentHandler newIdentifierAssignmentHandler(String terminology) throws Exception {
		return ConfigUtility.newStandardHandlerInstanceWithConfiguration("identifier.assignment.handler", terminology,
				IdentifierAssignmentHandler.class);
	}
	...
	/* see superclass */
	@Override
	public String getComputedPreferredName(AtomClass atomClass, PrecedenceList list) throws Exception {
		try {
	...
	/* see superclass */
	@Override
	public Map<String, Integer> getComponentStats(String terminology, String version, String branch) throws Exception {
		...
		for (final EntityType<?> type : manager.getMetamodel().getEntities()) {
			...
			if (terminology != null) {
				query = manager.createQuery("select count(*) from " + jpaTable + " where terminology = :terminology "
						+ "and version = :version ");
				query.setParameter("terminology", terminology);
				query.setParameter("version", version);
	 ...
	 */
	@SuppressWarnings("rawtypes")
	protected <T extends Component> List getComponents(String terminologyId, String terminology, String version,
			Class<T> clazz) {
		try {
	...
	protected <T extends Component> void updateComponent(T newComponent) throws Exception {
		...
		if (isMolecularActionFlag()) {
			...
			final List<Method> columnMethods = IndexUtility.getAllColumnGetMethods(oldComponent.getClass());
			...
			for (final Method m : columnMethods) {
				...
				final String oldValue = (oldObject == null ? "" : m.invoke(oldComponent, new Object[] {}).toString());
				final String newValue = (newObject == null ? "" : m.invoke(newComponent, new Object[] {}).toString());
			...
			final List<Method> oneToManyMethods = IndexUtility.getAllCollectionGetMethods(oldComponent.getClass());

			// Iterate through @OneToMan methods
			for (final Method m : oneToManyMethods) {

				// Obtain the old/new identifier lists
				final List<?> oldList = new ArrayList<>((Collection<?>) m.invoke(oldComponent, new Object[] {}));
				final Set<Long> oldIds = (oldList.stream().map(x -> ((HasId) x).getId()).collect(Collectors.toSet()));

				final List<?> newList = new ArrayList<>((Collection<?>) m.invoke(newComponent, new Object[] {}));
				final Set<Long> newIds = (newList.stream().map(x -> ((HasId) x).getId()).collect(Collectors.toSet()));
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected <T extends Component> T getComponent(String terminologyId, String terminology, String version,
			String branch, Class<T> clazz) {
	...
	/* see superclass */
	@Override
	public RelationshipList findConceptRelationships(String conceptId, String terminology, String version,
			String branch, String query, boolean inverseFlag, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find relationships for concept " + conceptId + "/"
				+ terminology + "/" + version + "/" + branch + "/" + query + "/" + inverseFlag);

		return findRelationshipsForComponentHelper(conceptId, terminology, version, branch, query, inverseFlag, pfs,
				ConceptRelationshipJpa.class);
	}

	/* see superclass */
	@Override
	public RelationshipList findComponentInfoRelationships(String componentInfoId, String terminology, String version,
			IdType type, String branch, String query, boolean inverseFlag, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find relationships for component info " + componentInfoId
				+ "/" + terminology + "/" + version + "/" + branch + "/" + query + "/" + inverseFlag);

		if (inverseFlag) {
			return findRelationshipsForComponentHelper(componentInfoId, terminology, version, branch,
					ConfigUtility.isEmpty(query) ? "toType:" + type : query + " AND toType:" + type, inverseFlag, pfs,
					ComponentInfoRelationshipJpa.class);

		} else {
			return findRelationshipsForComponentHelper(componentInfoId, terminology, version, branch,
					ConfigUtility.isEmpty(query) ? "fromType:" + type : query + " AND fromType:" + type, inverseFlag,
					pfs, ComponentInfoRelationshipJpa.class);
		}
	}
	...
	@SuppressWarnings({ "unchecked" })
	@Override
	public RelationshipList findConceptDeepRelationships(String conceptId, String terminology, String version,
			String branch, String filter, boolean inverseFlag, boolean includeConceptRels, boolean preferredOnly,
		...
		Logger.getLogger(getClass()).debug("Content Service - find deep relationships for concept " + conceptId + "/"
		...
		if (ConfigUtility.isEmpty(suppressibleClause) && pfs != null && pfs.getQueryRestriction() != null
		...
		try {
			...
			if (includeConceptRels) {
				queryStr = "select a.id, a.terminologyId, a.terminology, a.version, "
						...
						+ (inverseFlag ? "a.to" : "a.from") + ".id = :conceptId " + relTypeClause + suppressibleClause;
			...
			queryStr = "select a.id, a.terminologyId, a.terminology, a.version, "
					...
					+ (inverseFlag ? "a.to" : "a.from") + ".id in (:atomIds)" + relTypeClause + suppressibleClause;
			query = manager.createQuery(queryStr);
			query.setParameter("terminology", terminology);
			query.setParameter("version", version);
			...
			queryStr = "select a.id, a.terminologyId, a.terminology, a.version, "
					...
					+ (inverseFlag ? "e.id in (ca.id) " : "e.id in (ca.id) ") + relTypeClause + suppressibleClause;
			query = manager.createQuery(queryStr);
			query.setParameter("terminology", terminology);
			query.setParameter("version", version);
			query.setParameter("atomIds", atomIds);
			...
			for (final Object[] result : results) {
				...
				if (includeSelfReferential || !relationship.getFrom().getId().equals(relationship.getTo().getId())) {
	...
	/* see superclass */
	@Override
	public RelationshipList findDescriptorRelationships(String descriptorId, String terminology, String version,
			String branch, String query, boolean inverseFlag, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find relationships for descriptor " + descriptorId + "/"
				+ terminology + "/" + version + "/" + branch + "/" + query + "/" + inverseFlag);

		return findRelationshipsForComponentHelper(descriptorId, terminology, version, branch, query, inverseFlag, pfs,
	...
	/* see superclass */
	@Override
	public RelationshipList findCodeRelationships(String codeId, String terminology, String version, String branch,
			String query, boolean inverseFlag, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find relationships for code " + codeId + "/" + terminology
				+ "/" + version + "/" + branch + "/" + query + "/" + inverseFlag);

		return findRelationshipsForComponentHelper(codeId, terminology, version, branch, query, inverseFlag, pfs,
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private RelationshipList findRelationshipsForComponentHelper(String terminologyId, String terminology,
		...
		results.setObjects((List) searchHandler.getQueryResults("", "", branch, finalQuery, "toNameSort", clazz, pfs,
				totalCt, manager));
		results.setTotalCount(totalCt[0]);

		for (final Relationship<? extends ComponentInfo, ? extends ComponentInfo> rel : results.getObjects()) {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private MappingList findMappingsForComponentHelper(String terminologyId, String terminology, String version,
		...
		finalQuery.append("fromTerminologyId:" + QueryParserBase.escape(terminologyId) + " AND fromTerminology:"
				+ terminology + " AND fromVersion:" + version);

		final SearchHandler searchHandler = getSearchHandler(terminology);
		final int[] totalCt = new int[1];
		// pass empty terminology/version because it's handled above
		results.setObjects((List) searchHandler.getQueryResults("", "", branch, finalQuery.toString(), "fromNameSort",
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private TreePositionList findTreePositionsHelper(String terminologyId, String terminology, String version,
			String branch, String query, PfsParameter pfs, Class<? extends TreePosition> clazz) throws Exception {
		...
		list.setObjects((List) searchHandler.getQueryResults(terminology, version, branch, finalQuery.toString(),
				"nodeNameSort", clazz, pfs, totalCt, manager));
		list.setTotalCount(totalCt[0]);
	...
	/* see superclass */
	@Override
	public SearchResultList findCodesForGeneralQuery(String luceneQuery, String JPQLQuery, String branch,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).info("Content Service - find codes " + luceneQuery + "/" + JPQLQuery + "/");
	...
	@Override
	public SearchResultList findConceptsForGeneralQuery(String luceneQuery, String JPQLQuery, String branch,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).info("Content Service - find concepts " + luceneQuery + "/" + JPQLQuery + "/");
	...
	@Override
	public SearchResultList findDescriptorsForGeneralQuery(String luceneQuery, String JPQLQuery, String branch,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).info("Content Service - find descriptors " + luceneQuery + "/" + JPQLQuery + "/");
	...
	public Tree getTreeForTreePosition(TreePosition<?> treePosition) throws Exception {
		...
		for (final String pathPart : fullAncPath.split("~")) {
			...
			final StringBuilder finalQuery = new StringBuilder();
			finalQuery.append("nodeId:" + partId + " AND terminology:" + treePosition.getTerminology() + " AND version:"
			...
			final FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, clazz);
			...
			int ct = 0;
			final String treePositionRela = ConfigUtility.isEmpty(treePosition.getAdditionalRelationshipType()) ? ""
			...
			if (ct != 1) {
				throw new Exception(
						"Unexpected number of results: " + ct + ", " + partId + ", " + partAncPath + ", " + clazz);
			}
	...
	@SuppressWarnings("rawtypes")
	@Override
	public TreePositionList findTreePositions(String terminologyId, String terminology, String version, String branch,
			String query, Class<? extends TreePosition> clazz, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).info("Content Service - find concept tree positions " + terminologyId + "/"
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public TreePositionList findTreePositionChildren(Long nodeId, String terminologyId, String terminology,
			String version, String branch, Class<? extends TreePosition> clazz, PfsParameter pfs) throws Exception {
		...
		if (nodeId != null) {
			tpList = findTreePositionsHelper(null, null, null, branch, "nodeId:" + nodeId, childPfs, clazz);
		} else {
			tpList = findTreePositionsHelper(terminologyId, terminology, version, branch, "", childPfs, clazz);
		}
		if (tpList.size() == 0) {
		...
		final TreePositionList list = new TreePositionListJpa();
		list.setObjects((List) searchHandler.getQueryResults(terminology, version, branch, query, null, clazz, pfs,
				totalCt, manager));
		list.setTotalCount(totalCt[0]);
	...
	/* see superclass */
	@Override
	public GeneralConceptAxiomList getGeneralConceptAxioms(String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass())
				.debug("Content Service - get general concept axioms " + terminology + "/" + version);
		final javax.persistence.Query query = manager.createQuery(
				"select a from GeneralConceptAxiomJpa a where " + "version = :version and terminology = :terminology");
		// Try to retrieve the single expected result If zero or more than one
		// result are returned, log error and set result to null
		try {
			query.setParameter("terminology", terminology);
	...
	public void updateMapping(Mapping mapping) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(mapping.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
	...
	/* see superclass */
	@Override
	public MapSet getMapSet(String terminologyId, String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get mapset " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
	...
	public MapSetList getMapSets(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - get mapsets " + terminology + "/" + version);
	...
	public Mapping getMapping(String terminologyId, String terminology, String version, String branch)
			throws Exception {
		Logger.getLogger(getClass()).debug(
				"Content Service - get mapping " + terminologyId + "/" + terminology + "/" + version + "/" + branch);
	...
	public MappingList findMappings(Long mapSetId, String query, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find mappings " + mapSetId + ", query=" + query);
		...
		int[] totalCt = new int[1];
		final List<Mapping> list = (List<Mapping>) getQueryResults(sb.toString(), MappingJpa.class, pfs, totalCt);
	...
	/* see superclass */
	@Override
	public MappingList findConceptMappings(String conceptId, String terminology, String version, String branch,
			String query, PfsParameter pfs) throws Exception {
	...
	/* see superclass */
	@Override
	public MappingList findCodeMappings(String codeId, String terminology, String version, String branch, String query,
			PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find mappings for code " + codeId + "/" + terminology
				+ "/" + version + "/" + branch + "/" + query);
	...
	/* see superclass */
	@Override
	public MappingList findDescriptorMappings(String descriptorId, String terminology, String version, String branch,
			String query, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find mappings for descriptor " + descriptorId + "/"
				+ terminology + "/" + version + "/" + branch + "/" + query);
	...
	public void updateMapSet(MapSet mapSet) throws Exception {
		...
		// Id assignment should not change
		final IdentifierAssignmentHandler idHandler = getIdentifierAssignmentHandler(mapSet.getTerminology());
		if (assignIdentifiersFlag) {
			if (!idHandler.allowIdChangeOnUpdate()) {
	...
	/* see superclass */
	@Override
	public Map<Long, String> getTerminologyIdMap(String terminology, String version, String branch) throws Exception {
		Logger.getLogger(getClass())
		...
		javax.persistence.Query query = manager.createQuery("select a.id, a.terminologyId from " + tableName
	...
	public NoteList findConceptNotes(String query, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find concept notes " + query + ", " + pfs);
	...
	public NoteList findDescriptorNotes(String query, PfsParameter pfs) throws Exception {
		Logger.getLogger(getClass()).debug("Content Service - find description notes " + query + ", " + pfs);
		final NoteList results = new NoteListJpa();
		final SearchHandler searchHandler = getSearchHandler(null);
		final int[] totalCt = new int[1];
		final List<DescriptorNoteJpa> luceneResults = searchHandler.getQueryResults(null, null, "", query, "",
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public TreePositionList findConceptDeepTreePositions(String terminologyId, String terminology, String version,
			String branch, String query, PfsParameter pfs) throws Exception {

		Logger.getLogger(getClass()).debug("Content Service - find tree positions for concept " + terminologyId + "/"
				+ terminology + "/" + version + "/" + query);
		...
		final List<String> clauses = concept.getAtoms().stream().map(a -> "nodeId:" + a.getId())
		...
		for (final Atom atom : concept.getAtoms()) {
			...
			final String entry = type + ":" + atom.getTerminology() + ":" + atom.getVersion() + ":" + lterminologyId;
			...
			if (!seen.contains(entry)) {
				...
				final TreePosition<?> treePos = getFirstTreePosition(lterminologyId, atom.getTerminology(),
						atom.getVersion(), clazz);
				// Increment if so
				if (treePos != null) {
					// handle lazy init
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private TreePosition<?> getFirstTreePosition(String terminologyId, String terminology, String version,
		...
		final TreePositionList list = findTreePositions(terminologyId, terminology, version, null, null, clazz,
				singleResultPfs);
		if (list.size() > 0) {
	...
	/* see superclass */
	@Override
	public Set<Long> validateConcepts(Project project, String check, Set<Long> conceptIds) throws Exception {
		...
		for (final String key : getValidationHandlersMap().keySet()) {
			if (project.getValidationChecks().contains(key) && (check == null || check.equals(key))) {
				final Set<Long> failedCheck = getValidationHandlersMap().get(key).validateConcepts(conceptIds,
	...
	private void validateInit() throws Exception {
		if (idHandlerMap == null) {
			throw new Exception("Identifier assignment handler did not properly initialize, serious error.");
		}
		...
		if (!searchHandlerNames.contains(ConfigUtility.ATOMCLASS)) {
			throw new Exception("search.handler." + ConfigUtility.ATOMCLASS + " expected and does not exist.");
		}
	}
