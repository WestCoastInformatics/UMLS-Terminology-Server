...
public class GeneratedMergeAlgorithm extends AbstractMergeAlgorithm {
  ...
  public void compute() throws Exception {
    ...
    String algoName = "GENERATEDMERGE";
    String algoCamelName = "GeneratedMerge";
    // If this is being called by Mid-Merge, prepend the lastModifiedBy with
    // "ENG-", and change the algorithm name for logging
    if (midMerge) {
      setLastModifiedBy("ENG-" + getLastModifiedBy());
      algoName = "MIDMERGE";
      algoCamelName = "MidMerge";
    }

    logInfo("Starting " + algoName);
    ...
    try {
      ...
      logInfo(
          "[" + algoCamelName + "] " + statsMap.get("atomPairsReturnedByQuery")
              + " atom pairs were returned by the query.");
      logInfo(
          "[" + algoCamelName + "] " + statsMap.get("atomPairsRemovedByFilters")
              + " atom pairs were removed by filters.");
      logInfo("[" + algoCamelName + "] "
          + statsMap.get("atomPairsRemainingAfterFilters")
          + " atom pair merges attempted.");
      logInfo("[" + algoCamelName + "] " + statsMap.get("successfulMerges")
          + " merges successfully performed.");
      logInfo("[" + algoCamelName + "] " + statsMap.get("unsuccessfulMerges")
          + " attempted merges were unsuccessful.");
      if (makeDemotions) {
        logInfo("[" + algoCamelName + "] " + statsMap.get("successfulDemotions")
            + " demotions successfully created.");
        logInfo(
            "[" + algoCamelName + "] " + statsMap.get("unsuccessfulDemotions")
