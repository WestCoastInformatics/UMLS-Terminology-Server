...
cd ~/gene
unzip ~/gene/code/config/prod/target/term*.zip -d config
ln -s config/bin
...
cd ~/gene
echo "CREATE database genedb CHARACTER SET utf8 default collate utf8_unicode_ci;" | mysqlu
bin/load.csh
...
mysqlu < ~/gene/code/admin/mojo/src/main/resources/truncate_all.sql
mysqlu < ~/gene/data/gene.sql
...
unzip gene-indexes.zip -d /var/lib/tomcat8/indexes/gene
