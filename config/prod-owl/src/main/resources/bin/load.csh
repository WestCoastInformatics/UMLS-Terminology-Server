...
mvn install -POwl -Drun.config.owl=$UMLS_CONFIG -Dmode=create -Dserver=$SERVER -Dterminology=SNOMEDCT -Dversion=latest -Dinput.file=$UMLS_DATA/snomed.owl >&! mvn.log
if ($status != 0) then
...
cd $UMLS_CODE/admin/loader
mvn install -POwl -Drun.config.owl=$UMLS_CONFIG -Dserver=$SERVER -Dterminology=GO -Dversion=latest -Dinput.file=$UMLS_DATA/go.owl >&! mvn.log
if ($status != 0) then
...
cd $UMLS_CODE/admin/loader
mvn install -POwl -Drun.config.owl=$UMLS_CONFIG -Dserver=$SERVER -Dterminology=NCI -Dversion=latest -Dinput.file=$UMLS_DATA/Thesaurus.owl >&! mvn.log
if ($status != 0) then
